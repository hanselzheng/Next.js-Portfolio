...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('hanseldsusanto03@gmail.com');
  ...
  return (
    ...
    >
      <div className={`${id === 6 && 'flex justify-center'} h-full`}>
        <div className="w-full h-full flex-end absolute">
          {img && (
            <img 
              src={img}
              alt={img}
        ...
        {id === 3 && (
          <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
            <div className="flex flex-col gap-3 lg:gap-3">
              {['Kali Linux', 'Python', 'SQL'].map((item) => (
                <span key={item} className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-base opacitty-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">{item}</span>
              ))}
            ...
            <div className="flex flex-col gap-3 lg:gap-3">
              ...
              {['React.js', 'Next.js', 'HTML'].map((item) => (
                <span key={item} className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-base opacitty-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">{item}</span>
              ))}
            </div>
          </div>
